...
int Noise2(int x, int y, const int seed) {
  int yindex = (y + seed) % 256;
  if (yindex < 0) yindex += 256;
  int xindex = (kHash[yindex] + x) % 256;
  if (xindex < 0) xindex += 256;
  const int result = kHash[xindex];
  return result;
}

double LinInter(double x, double y, double s) { return x + s * (y - x); }

double SmoothInter(double x, double y, double s) {
  return LinInter(x, y, s * s * (3 - 2 * s));
}

double Noise2D(double x, double y, const int seed) {
  ...
  const double result = SmoothInter(low, high, y_frac);
  return result;
}

double Get2D(double x, double y, double freq, int depth, const int seed) {
  double xa = x * freq;
  double ya = y * freq;
  double amp = 1.0;
  double fin = 0;
